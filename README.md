...
Performance is key. The API proxy uses asynchronous logging, rate limiting is done in memory using a simple and efficient [algorithm](http://en.wikipedia.org/wiki/Token_bucket), API key enforcement is done in memory, and proxying is done by the ["battle-hardened node.js http proxy."]
